...
int FileSynchronize::XmlParse(boost::shared_ptr<Initialize> init)
{
    ...
    if (rmtServers.size())
    {
        ...
        //cout << "同步文件时开启的最大线程数为: " << 2 + init->sync_num + (rmtServers.size( )) * init->sync_num << "  = " << 2 + init->sync_num << "(等待时开启的线程数) + "\
        //<< (rmtServers.size( ))*(init->sync_num) << "(子同步线程)" << endl;
...
void* FileSynchronize::RsyncThread(ptrQRetry qr, Event event, string ip, string module, string watch)
{
    ...
    int dir = event->dir;
    string path;
    string command = "cd " + watch + " && rsync -artuz -R " + ssh + port + timeout;
    ...
    if (res && res != 5888)// 5888 is the error no permitted to  set times  but the file has been rsynced
    {
        qr->push(command);
    }
...
bool FileSynchronize::RsyncOnce()
{
    ...
    {
        string command = "cd " + watch + " && rsync -artuz -R " + m_delete + " ./ " + ssh + port + timeout;
        ...
        if (debug == true) cout << "crontab command:" << command << endl;
        system(command.c_str());
    }
}
